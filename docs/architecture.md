...
## Наблюдаемость: трейсинг (шаг 9-10b, сделано)

OpenTelemetry Java agent + Jaeger. Агент, а не Spring Boot's Micrometer
Tracing bridge (`micrometer-tracing-bridge-otel` +
`opentelemetry-exporter-otlp`) — тот путь идёт через автоконфигурацию
самого Boot, а это ровно та поверхность, что уже дважды преподносила
сюрпризы под Boot 4 (префикс Mongo-свойств на шаге 5, пакет
`ConcurrentKafkaListenerContainerFactory` на шаге 4, тот же довод — почему
в шаге 8 выбрали core-библиотеки Resilience4j вместо стартера). Агент
инструментирует байткод на уровне JVM, не завязан на то, какая версия
Spring Boot запущена — включая проброс trace context через ЗАГОЛОВКИ
Kafka-сообщений автоматически.

Подключается через `-javaagent:` в Dockerfile каждого сервиса (jar
скачивается на этапе сборки), конфигурация — только переменные
окружения (`OTEL_SERVICE_NAME`, `OTEL_EXPORTER_OTLP_ENDPOINT`,
`OTEL_METRICS_EXPORTER=none`/`OTEL_LOGS_EXPORTER=none` — метрики уже
покрыты Prometheus/Grafana с шага 9-10a, второй, дублирующий пайплайн
метрик через OTel только добавил бы путаницы).

**Два реальных гоча, оба пойманы вживую до того, как стали проблемой**:

1. Порт агент по умолчанию говорит по `http/protobuf`, не по gRPC — сам
   агент честно предупреждает в логе, если endpoint указывает на "не
   тот" порт для выбранного протокола (`http://jaeger:4317` — порт для
   gRPC — с протоколом по умолчанию http/protobuf, которому нужен 4318).
   Без этого предупреждения экспорт трейсов молча проваливался бы
   (`Connection reset by peer` в логе, ни одного спана в Jaeger).
2. Версия агента v2.10.0 (на момент написания — актуальная стабильная
   на вид) не создавала спанов для Kafka producer/consumer вообще —
   ни ошибки, ни предупреждения, просто тишина. Причина: `kafka-clients`
   в этом проекте — 4.2.1 (свежий мажорный релиз самой Kafka), а
   инструментация Kafka в агенте version-range'ится (muzzle-проверка) и
   молча пропускает версии клиента вне диапазона, который знает
   конкретный релиз агента. Тот же класс проблемы, что уже несколько раз
   ловился в этом проекте на стороне Spring Boot 4 — только теперь на
   стороне OTel-агента, версия которого отстала от версии Kafka. Решено
   переходом на v2.31.1 (актуальный релиз на момент проверки) — Kafka-
   спаны появились сразу.

**Живая проверка — самая наглядная во всём проекте**: один trace ID,
3 сервиса, 35 спанов, найден и открыт в Jaeger UI. Дерево спанов
показывает ровно то, что и должно быть по архитектуре: `subscription-
service: OutboxPoller.publishPending` → `subscription.events publish`
(SDK-спан продюсера) → `billing-service: subscription.events process`
(спан consumer'а, автоматически СВЯЗАННЫЙ с спаном продюсера через
заголовки Kafka-сообщения, а не начатый заново) → вся цепочка обработки
списания в billing-service → `billing.events publish` → снова
subscription-service (подтверждение/компенсация) → и notification-service,
слушающий тот же `subscription.events`, — всё ОДНИМ деревом, не тремя
разрозненными.

**Важная архитектурная граница, которую это вскрыло**: HTTP-запрос
`POST /subscriptions` и последующая публикация в Kafka — это ДВА разных
корня трейса, не один непрерывный. Причина в самом outbox-паттерне:
`OutboxPoller` публикует по расписанию (`@Scheduled`), в СВОЁМ потоке,
никак не связанном с потоком, обработавшим исходный HTTP-запрос — а
именно поток определяет, откуда SDK берёт trace context для нового
спана. Значит, честный сквозной трейс "от клика клиента до обработки в
billing-service" в этом проекте НЕ существует как один trace ID — только
"от публикации" и дальше. Это не баг и не то, что стоит чинить: это
прямая, честная цена паттерна transactional outbox, который сознательно
РАЗРЫВАЕТ синхронную причинно-следственную связь между "запись сделана"
и "событие опубликовано" ради надёжности (см. раздел про outbox в
`docs/architecture.md`) — трейсинг просто делает эту развязку видимой,
а не скрывает её.

## Kubernetes / kind (шаг 11a, сделано)

Namespace/ConfigMap/Secret/Deployment/Service + пробы для того же
распределённого стека, что и `docker-compose.yml` — манифесты и полная
инструкция в [`k8s/README.md`](../k8s/README.md). Осознанно только ядро
системы (Kafka, обе Postgres, Mongo, три сервиса) — observability-стек
шагов 9-10 остаётся docker-compose-only, разворачивать Prometheus/
Grafana/Jaeger второй раз на второй платформе — раздувание объёма без
дополнительной демонстрации того, что реально просили (Deployment/
Service/ConfigMap/Secret/probes/HPA). Ingress и HPA — шаг 11b, ещё не
сделан (нужны отдельные компоненты кластера: ingress-nginx,
metrics-server).

**Четыре реальные проблемы, ни одна не гипотетическая** — все с живой
проверкой и фиксом, полный разбор в `k8s/README.md`:

1. `kind` копирует `HTTP_PROXY`/`HTTPS_PROXY` из вызывающего шелла в
   контейнер ноды при создании кластера — если прокси висит на
   `127.0.0.1`, изнутри ноды (другое сетевое пространство имён) он
   недостижим. Все image pull'ы падали, пока адрес не заменили на
   `host.docker.internal`.
2. `kind load docker-image` для multi-arch образов (postgres/mongo/kafka)
   падал с `content digest ... not found` — `kind load` всегда
   импортирует `--all-platforms`, а обычный `docker pull` тянет только
   layers текущей платформы. Решено: не грузить эти образы вручную,
   дать ноде (после фикса прокси) тянуть их из интернета самой.
3. **Настоящий deadlock самоссылающегося single-broker Kafka** —
   собственный KRaft-контроллер брокера должен достучаться до САМОГО
   СЕБЯ через Service (`kafka:19093`), но обычный `ClusterIP` Service
   маршрутизирует только на READY endpoints — под не может стать ready,
   пока не завершит регистрацию сама с собой. `clusterIP: None` в
   одиночку не помог (DNS headless-сервиса по умолчанию ТОЖЕ гейтится
   readiness) — настоящий фикс: `publishNotReadyAddresses: true`.
4. Пробы Kafka/Mongo (`kafka-broker-api-versions.sh`, `mongosh`) —
   команды, поднимающие целый рантайм (JVM/Node.js) на каждый вызов —
   регулярно не укладывались в ДЕФОЛТНЫЙ `timeoutSeconds: 1` — реальный
   живой инцидент: здоровый брокер убивался и перезапускался снова и
   снова из-за таймаута самой команды проверки, а не из-за реальной
   проблемы с брокером.

Живая проверка после всех фиксов: полная покупка + отмена (сага с
компенсацией из шага 7) через `kubectl port-forward`, оба раза с
подтверждением конечного статуса напрямую в Postgres-поде — работает на
kind идентично docker-compose.

## Kubernetes: Ingress + HPA (шаг 11b, сделано)

Ingress (`k8s/09-ingress.yaml`) и HPA (`k8s/10-hpa.yaml`) поверх ядра из
шага 11a — полный разбор и команды в [`k8s/README.md`](../k8s/README.md).

**Ingress**: один `Ingress` на все три сервиса вместо host-based роутинга
(у kind всё равно один адрес) — маршрутизация по ПРЕФИКСУ пути
(`/subscription-service/...`, `/billing-service/...`,
`/notification-service/...`), а не по путям самого приложения напрямую,
потому что billing-service и subscription-service оба отвечают на
одинаковый `/admin/dlq/{topic}/replay` (шаг 6) — по одному этому пути
понять, какой сервис имелся в виду, нельзя. Префикс срезается аннотацией
`rewrite-target` с regex-группой захвата ДО того, как запрос доходит до
приложения — само приложение не подозревает, что префикс вообще
существовал. Требует `ingress-nginx` (в kind-варианте) и пересозданный
...
самого начала спроектированы безопасными под несколько реплик — HPA
здесь проверяет решение десятилетней (10-шаговой) давности, а не вводит
новое поведение. `averageUtilization: 50` считается относительно
`resources.requests.cpu`, поэтому запрос уже должен был существовать в
Deployment. Живая проверка получилась не постановочной: сразу после
применения манифестов HPA поймал реальный CPU-всплеск разогрева JVM
нового пода (Kafka consumer join, JPA, OTel javaagent) и САМ, без
искусственной нагрузки, поднял subscription-service с 1 до 4 реплик,
после чего вошёл в `ScaleDownStabilized` (кулдаун на снижение).

**Ещё одна реальная проблема, найдена и исправлена живьём**:
subscription-service и billing-service после пересоздания кластера ушли
в цикл перезапусков — `kubectl logs --previous` показал `Started
SubscriptionServiceApplication in 55.091 seconds`, а через секунду сразу
`Commencing graceful shutdown`. Причина — `livenessProbe` с
`initialDelaySeconds: 30` и дефолтным `failureThreshold: 3` даёт ~75с
форы; под нагрузкой на CPU от самого kind (несколько сервисов, Kafka,
OTel-агент разбирает байткод при старте) этот JVM реально стартовал 55-75с
— значит liveness-проба уже трижды провалилась и kubelet прислал SIGTERM
буквально в момент, когда сервис только-только успел подняться. Разница
с граблей #4 из шага 11a (таймаут ОДНОЙ проверки): здесь ломался не сам
запрос проверки, а суммарное окно ДО первого успеха. Фикс — не тот же
`timeoutSeconds`, а `initialDelaySeconds: 60` + `failureThreshold: 5`
(итого ~135с форы) на liveness обоих сервисов; readiness не трогали —
её провалы не убивают под, только не пускают трафик, так что риска в
той же тесноте нет.
